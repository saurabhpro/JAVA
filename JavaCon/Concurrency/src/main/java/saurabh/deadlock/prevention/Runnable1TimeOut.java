...
public record Runnable1TimeOut(Lock lock1, Lock lock2) implements Runnable {
    ...
    private boolean tryLockBothLocks() {
        ...
        try {
            boolean lock1Succeeded = lock1.tryLock(100, TimeUnit.MILLISECONDS);
            if (!lock1Succeeded) {
                return false;
            }
        } catch (InterruptedException e) {
        ...
        try {
            boolean lock2Succeeded = lock2.tryLock(100, TimeUnit.MILLISECONDS);
            if (!lock2Succeeded) {
                lock1.unlock();
                return false;
            }
