...
class LambdaTypes {
	static void f1lambda() {
		...
		/*Overriding method from Interface functionalInterface.F1
		 * from the code it is understood that RHS code returns a reference just like anonymous class
		 *
		 * the compiler sees the () and immediately knows we are taking about void show() of functionalInterface.F1
	...
	static void f3Lambda() {
		...
		System.out.println("functionalInterface.F3\t" + f31.calculate(5, 9));
...
public class LambdaWrittenTypes {
	...
	public static void main(String[] args) {
		 ...
		 * In this statement, the compiler finds that the right-hand side of the assignment operator is a lambda
		 ...
		 functionalInterface.StringToIntMapper interface; it verifies that the lambda expression conforms to the declaration of the map() method
		 in the functionalInterface.StringToIntMapper interface;
		 finally, it infers that the type of the lambda expression is the functionalInterface.StringToIntMapper interface type.
