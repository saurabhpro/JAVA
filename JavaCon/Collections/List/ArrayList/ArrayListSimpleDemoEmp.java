...
public class ArrayListSimpleDemoEmp {
	...
	void ownClassRemoveWithEquals() {
		/**
		 * if you want to perform any operation on any class object in ArrayList then that class (Say Emp)
		...
		while (i.hasNext()) {
			Emp z = i.next();
			//next returns Object, as we already fixed type to <Emp> in Iterator we dont need to downcast it
			//else   Emp z = (Emp)i.next();
			System.out.println(z.id + " " + z.name);
		}
