...
public class SimpleDemoEmp {
	...
	void ownClassRemoveWithEquals() {
		...
		while (i.hasNext()) {
			Emp z = i.next();
			//next returns Object, as we already fixed type to <main.java.list.arraylist.model.Emp> in Iterator we dont need to downcast it
			//else   main.java.list.arraylist.model.Emp z = (main.java.list.arraylist.model.Emp)i.next();
			System.out.println(z.id + " " + z.name);
		}
