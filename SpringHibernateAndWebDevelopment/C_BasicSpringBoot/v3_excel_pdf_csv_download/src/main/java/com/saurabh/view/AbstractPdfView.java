...
/**
 * Spring provides an AbstractPdfView abstract class which can be subclassed
 * to create a helper class for generating PDF documents. However, it has a big
 * drawback which the AbstractPdfView class only supports old API version of iText
 * i.e. it is using the package com.lowagie.*(iText version <= 2.1.7) while the
 * recent iText’s package changes to com.itextpdf.* (iText version >= 5.x)
 * <p>
 * The old iText version is no longer available nor supported, so subclassing
 * AbstractPdfView class is discouraged. Instead, I recommend to subclass the
 * AbstractView class to create an iText 5.x-compatible version.
 */
