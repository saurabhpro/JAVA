 ...
 * @version 1.0 Class to find discrepancy and set the discrepancy flag
 */
...
public class MarkDiscrepancy extends FinalObject {
	...
	public final void findDiscrepancy() {
		// MarkDiscrepancy if an employeemodal is absent and there is no entry
		// in Hrnet file.
		for (FinalObjectModel finalObjectModel : EmpCombinedMap.values())

			if (finalObjectModel.attendanceOfLeaves == null)
				MarkDiscrepancyHelperUtility.checkAbsentButNoLeaveApplied(finalObjectModel);
			else /* case where there is an entry */
				MarkDiscrepancyHelperUtility.checkIfEntryPresentInBothBiometricAndSalesForce(finalObjectModel);
	}
}
