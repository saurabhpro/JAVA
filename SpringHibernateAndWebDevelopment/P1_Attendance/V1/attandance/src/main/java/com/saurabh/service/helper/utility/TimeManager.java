...
public final class TimeManager {
	 ...
	 * @param attendanceOfADate Object that contains all details for each day of
	 *                          month specified in Project Constants
	 * @return the average calculated in LocalTime format
	 * @see
	 */
	public static LocalTime calculateAverageOfTime(String type, AttendanceOfADate[] attendanceOfADate) {
		List<AttendanceOfADate> ofDates = Arrays.asList(attendanceOfADate);
		int hoursTotal, minsTotal = 0, presentDays = 0;

		for (AttendanceOfADate date : ofDates) {
			if (date.getCheckIn() != null && !date.getCheckOut().equals(LocalTime.MIDNIGHT)) {
				switch (type) {
					case ProjectConstants.EMP_AVERAGE_MONTHLY_CHECK_IN:
					...
					case ProjectConstants.EMP_AVERAGE_MONTHLY_CHECK_OUT:
	 ...
	 * @see
	 */
	public static LocalTime calculateAverageTimeOfMonth(AttendanceOfADate[] attendanceOfADate) {
		List<AttendanceOfADate> ofDates = Arrays.asList(attendanceOfADate);
		int hoursTotal, minsTotal = 0, presentDays = 0;
	...
	private static LocalTime getTime(LocalDateTime fromDateTime, LocalDateTime toDateTime) {
		...
		AtomicLong hours = new AtomicLong(seconds.get() / ProjectConstants.SECONDS_PER_HOUR);
		AtomicLong minutes = new AtomicLong(((seconds.get() % ProjectConstants.SECONDS_PER_HOUR) / ProjectConstants.SECONDS_PER_MINUTE));
